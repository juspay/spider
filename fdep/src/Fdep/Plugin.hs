...
import GHC.Hs.Binds
import Prelude hiding (id,writeFile, mapM)
import Data.Aeson
...
import GHC.Plugins (getOccString, pprNameSpaceBrief, occNameSpace, occNameString, occNameFS)
import Language.Haskell.Syntax.Pat (LHsRecUpdField)
import Language.Haskell.Syntax.Expr (LHsRecUpdProj, FieldLabelStrings)
import GHC.Types.Name.Reader (RdrName(..))
import Streamly.Prelude (drain, fromList, mapM, mapM_, toList)
import Streamly (serially,parallely)
import Data.Data ( Data(toConstr) )
import Data.Maybe (catMaybes)
import GHC.Hs.Type (rdrNameAmbiguousFieldOcc)
import GHC.Data.FastString (unpackFS)
import GHC.Utils.Outputable (pprPrefixOcc)
...
getAllTypeManipulations binds = do
  ...
  where
    ...
    getDataTypeDetails (RecordCon _ (L _ conLike) rcon_flds) = Just (TypeVsFields (nameStableString $ conLikeName conLike) (extractRecordBinds (rcon_flds)))
    ...
    inferFieldTypeAFieldOccP = (showSDocUnsafe . ppr . unLoc)
    ...
    handleRdrName x = 
        case x of 
            ...
            Orig module' occName    -> ((moduleNameString $ mkModuleName $ moduleStableString module') <> "$" <> (showSDocUnsafe $ pprNameSpaceBrief $ occNameSpace occName) <>  "$" <> (occNameString occName) <> "$" <> (unpackFS $ occNameFS occName))
            Exact name              -> nameStableString name

    getFieldUpdates :: Either [LHsRecUpdField GhcTc] [LHsRecUpdProj GhcTc] -> [FieldRep]
    getFieldUpdates (Left fields) = map extractField fields
      where
    ...
    getFieldUpdates (Right fields) = map extractProj fields
      where
        extractProj :: LHsRecUpdProj GhcTc -> FieldRep
        extractProj (L _ (HsRecField {hsRecFieldLbl = lbl, hsRecFieldArg = expr, hsRecPun = pun})) =
            if pun
                then FieldRep (showSDocUnsafe $ ppr lbl) (showSDocUnsafe $ ppr lbl) (inferFieldTypeAFieldOccP lbl)
                else FieldRep (showSDocUnsafe $ ppr lbl) (showSDocUnsafe $ ppr (unLoc expr)) (inferFieldTypeAFieldOccP lbl)
    ...
    getFunctionName (L _ x@(FunBind fun_ext id matches _)) = [nameStableString $ getName id]
    getFunctionName (L _ (VarBind{var_id = var, var_rhs = expr})) = [nameStableString $ getName var]
...
loopOverLHsBindLR (L _ x@(FunBind fun_ext id matches _)) = do
...
processArgs (funr) = case funr of
    (HsUnboundVar _ uv) -> [showSDocUnsafe $ pprPrefixOcc uv]
    (HsConLikeOut _ c) -> [showSDocUnsafe $ pprPrefixOcc c]
    (HsIPVar _ v) -> [showSDocUnsafe $ ppr v]
    (HsOverLabel _ l) -> [showSDocUnsafe $ ppr l]
...
processExpr arguments x@(L _ (HsVar _ (L _ var))) =
    let name = nameStableString $ getName var
        _type = showSDocUnsafe $ ppr $ getName var
...
processExpr arguments (L _ (HsStatic _ fun)) =
    processExpr arguments fun
processExpr arguments (L loc (XExpr (WrapExpr (HsWrap _ fun)))) =
    let r = processArgs fun
     in processExpr (arguments <> r) (L loc fun)
processExpr arguments (L _ (HsBinTick _ _ _ fun)) =
    processExpr arguments fun
processExpr arguments (L _ (ExplicitList _ funList)) =
...
processExpr arguments (L _ (HsIf exprLStmt funl funm funr)) =
    let stmts = (exprLStmt ^? biplateRef :: [LHsExpr GhcTc])
...
processExpr arguments (L _ (HsRnBracketOut _ exprLStmtL exprLStmtR)) =
    let stmtsL = (exprLStmtL ^? biplateRef :: [LHsExpr GhcTc])
        stmtsR = (exprLStmtR ^? biplateRef :: [LHsExpr GhcTc])
     in nub (concatMap (processExpr arguments) (stmtsL <> stmtsR))
processExpr arguments (L _ (HsTcBracketOut _ _ exprLStmtL exprLStmtR)) =
    let stmtsL = (exprLStmtL ^? biplateRef :: [LHsExpr GhcTc])
        stmtsR = (exprLStmtR ^? biplateRef :: [LHsExpr GhcTc])
     in nub (concatMap (processExpr arguments) (stmtsL <> stmtsR))
